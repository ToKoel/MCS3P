...
std::string Measurement::generateOutputHeader() {
  ...
  case MeasurementType::kMvsT:
    ...
    stream << "# dipole interactions: "
           << DipoleInteractionTypes[static_cast<int>(
...
std::string Measurement::generateOutputFilename() {
  ...
  case MeasurementType::kMvsT:
    outputFilename +=
        "_MvsT_sim_cF" + to_string(measurementSettings.coolingField, 2);
    outputFilename +=
        "T_mF" + to_string(measurementSettings.measurementField, 3);
    outputFilename +=
        "T_" + to_string(measurementSettings.monteCarloSteps, 1) + "steps";
    outputFilename +=
        "_" + to_string(measurementSettings.averagingSteps, 1) + "avsteps";
    outputFilename +=
        "_" + to_string(measurementSettings.numOrientations, 1) + "or";
    outputFilename += "_" + to_string(measurementSettings.sigma, 2) + "sig";
    break;

  case MeasurementType::kMvsH:
    outputFilename +=
        "_Hysteresis_sim_" + to_string(measurementSettings.temperature, 1);
    outputFilename +=
        "K_" + to_string(measurementSettings.monteCarloSteps, 1) + "steps";
    outputFilename +=
        "_" + to_string(measurementSettings.numOrientations, 1) + "or";
    outputFilename +=
        "_" + to_string(measurementSettings.coolingField, 2) + "T";
    outputFilename +=
        "_" + to_string(measurementSettings.coolingSteps, 2) + "cs";
    outputFilename +=
        "_sT" + to_string(measurementSettings.startingTemperature, 1) + "K";
    outputFilename +=
        "_dip" + DipoleInteractionTypes[static_cast<int>(
                     measurementSettings.dipoleInteractionHandling)];
    break;

  case MeasurementType::kSpinStructure:
    outputFilename +=
        "_spin_structure_" + to_string(measurementSettings.temperature, 1);
    outputFilename +=
        "K_" + to_string(measurementSettings.monteCarloSteps, 1) + "MCS";
    outputFilename +=
        "_" + to_string(measurementSettings.measurementField, 1) + "T";
    outputFilename +=
        "_dip" + DipoleInteractionTypes[static_cast<int>(
                     measurementSettings.dipoleInteractionHandling)];
    break;
  ...
  if (measurementSettings.dipoleInteractionHandling ==
      DipoleInteractions::kMacrocellMethod) {
    outputFilename +=
        "_" + to_string(measurementSettings.macrocellSize, 4) + "mcsize";
  }
...
void Measurement::temperatureSweep(
    ...
    std::vector<LinalgVector> &magnetizationArray) {
  ...
  for (auto i = 0; i < temperatureArray.size(); i++) {
    ...
    magnetizationArray[i] +=
        ...
        (static_cast<double>(totalNumAtoms *
                             measurementSettings.numOrientations));
  }
}
...
void Measurement::fieldSweep(Crystal &crystal,
  ...
  for (int i = 0; i < temperatureArray.size(); i++) {
    crystal.performMonteCarloSteps(
        measurementSettings.coolingSteps * totalNumAtoms,
        measurementSettings.coolingField, temperatureArray[i]);
  }

  for (int i = 0; i < fieldArray.size(); i++) {
    crystal.performMonteCarloSteps(
        measurementSettings.coolingSteps * totalNumAtoms, fieldArray[i],
...
void Measurement::writeSweepResultsToFile(
    MeasurementMode mode, std::vector<double> &variableArray,
    std::vector<LinalgVector> &magnetizationArray) {
  std::ofstream sweep;
  switch (mode) {
...
void Measurement::run_MvsT() {
  ...
  auto temperatureStepsCooling =
      arange<double>(measurementSettings.upperTemperatureLimit,
                     measurementSettings.lowerTemperatureLimit,
                     measurementSettings.temperatureStepSize);
  auto temperatureStepsHeating =
      arange<double>(measurementSettings.lowerTemperatureLimit,
                     measurementSettings.upperTemperatureLimit,
                     measurementSettings.temperatureStepSize);
  ...
  for (int i = 0; i < measurementSettings.numOrientations; i++) {
    ...
    if (measurementSettings.ZFC) {
      ...
      writeSweepResultsToFile(MeasurementMode::ZFC, temperatureStepsHeating,
                              magnetizationZFC);
    }

    if (measurementSettings.FC) {
      temperatureSweep(crystal, temperatureStepsCooling, magnetizationFC);
      writeSweepResultsToFile(MeasurementMode::FC, temperatureStepsCooling,
                              magnetizationFC);
    }
  }
}
...
void Measurement::run_MvsB() {
  ...
  std::vector<double> fieldArray =
      arange<double>(0, measurementSettings.upperFieldLimit,
                     measurementSettings.fieldStepSize);
  std::vector<double> fieldArrayUp = arange<double>(
      measurementSettings.lowerFieldLimit, measurementSettings.upperFieldLimit,
      measurementSettings.fieldStepSize);
  std::vector<double> fieldArrayDown = arange<double>(
      measurementSettings.upperFieldLimit, measurementSettings.lowerFieldLimit,
      measurementSettings.fieldStepSize);
  std::vector<double> temperatureArray = arange<double>(
...
void Measurement::run_spinstructure() {
  ...
  std::vector<double> coolingArray =
      arange(measurementSettings.startingTemperature,
