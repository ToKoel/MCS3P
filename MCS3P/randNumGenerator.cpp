...
/*
Random numbder generator was developed by Ralf Meyer and Fred Hucht (Copyright
1995 Ralf Meyer, 47058 Duisburg, Germany), but was slightly adapted for the use
in this program.
*/

long rnd250() {
  Rnd250.point = (Rnd250.point + 1) & 255;
  Rnd250.field[Rnd250.point] = Rnd250.field[(Rnd250.point - 250) & 255] ^
                               Rnd250.field[(Rnd250.point - 103) & 255];
  return Rnd250.field[Rnd250.point];
}
...
void seed250(long seed) {
  ...
  j = 0x40000000;
  for (i = 1; i < 250; i += 8) {
    Rnd250.field[i] = (Rnd250.field[i] & k) | j;
  }
...
utility::LinalgVector marsaglia() {
  ...
  do {
    y1 = rand0001_0999() * 2.0 - 1.0;
    y2 = rand0001_0999() * 2.0 - 1.0;
    rsq = y1 * y1 + y2 * y2;
  } while (rsq > 1.0);

  return {2.0 * y1 * std::sqrt((1.0 - rsq)), 2.0 * y2 * std::sqrt((1.0 - rsq)),
          1.0 - 2.0 * rsq};
}

int rand0_crystalSize(int dimension) {
  return (rnd250() % (dimension * dimension * dimension * 24));
}

int rand0_crystalAtoms(int totalNumAtoms) {
  return (rnd250() % (totalNumAtoms));
}

double rand0_1() // generate a random number in [0,1]
{
  return ((double)rnd250() / (double)RND250_MAX);
}

double rand0_90() {
  std::cout << (rnd250() % 91) << std::endl;
  return (rnd250() % 91);
}
...
static const double ytab[128] = {1,
                                 0.963598623011,
                                 0.936280813353,
                                 0.913041104253,
                                 0.892278506696,
...
double gaussian_ziggurat() {
  ...
  while (1) {
    ...
    if (i < 127) {
      ...
      x = PARAM_R - log(1.0 - rand0_1()) / PARAM_R;
      y = exp(-PARAM_R * (x - 0.5 * PARAM_R)) * rand0_1();
    }
    if (y < exp(-0.5 * x * x))
      break;
  }
